...
def generate_barcode(name, id):
    """
    Generate a Code39 barcode for the given ID and save it as an image.

    :param str name: Name associated with the barcode.
    :param str id: Unique ID for the barcode.
    """
    barcode_name = f"{name}_{id}"
    my_code = Code39(id, add_checksum=False, writer=ImageWriter())
    my_code.save(f"Barcodes/{barcode_name}")
